...
- Status: Workers Free diagnostic deployed on 2026-09-22. Live R2 → Queue → consumer → R2 cleanup and hosted Supabase signup/signin/signout passed. Some homepage/auth invocations exceeded the nominal 10 ms CPU budget without runtime errors; broader Free-plan suitability is not established.
...
- Hosted authentication passed after the user supplied an approved test mailbox through the ignored local `.env` file and confirmed the signup email. Signup → 302 confirmation page; wrong-password signin → 302 error redirect; correct-password signin → 302 `/`; authenticated dashboard → 200; signout → 302 `/`; subsequent dashboard → 302 `/auth/signin`. The hosted public settings endpoint also confirmed signup enabled and email confirmation required. The temporary password and session cookies were never printed or committed; no production auth settings were weakened.
- Later named web tails also recorded successful homepage/auth invocations at 13–15 ms CPU (including wrong-password signin at 15 ms and authenticated dashboard at 13 ms). All observed outcomes remained `ok` with zero exceptions, but these repeated samples reinforce that the wider Astro/auth application has not demonstrated a reliable 10 ms budget. The narrow diagnostic results must not be generalized to all app traffic. Investigate initialization/auth overhead before broader rollout; do not silently activate Paid.
